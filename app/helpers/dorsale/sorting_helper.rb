module Dorsale::SortingHelper
  def sortable_column(name, column)
    ...
    current_sort_param = params[:sort].to_s

    if current_sort_param.present?
      if current_sort_param.start_with?("-")
        current_column = current_sort_param[1..-1]
        current_order  = :desc
      else
        current_column = current_sort_param
        current_order  = :asc
      end
    end
    ...
    if current_column == column.to_s
      ...
      klass = ["sort", current_order].compact.join(" ")
    else
      new_sort_param = column
      klass          = "sort"
    end
